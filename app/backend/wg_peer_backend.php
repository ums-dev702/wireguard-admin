...
if (isset($_POST['edit_public_key'])) {
    ...
    try {
        ...
        header('Location: ../wg-peers.php?interface=' . urlencode($interface) . '&success=' . urlencode($success_message));
        exit;
    } catch (Exception $e) {
        error_log("Error updating public key: " . $e->getMessage());
        header('Location: ../wg-peers.php?interface=' . urlencode($interface) . '&error=Failed to update public key: ' . $e->getMessage());
        exit;
    }
}
